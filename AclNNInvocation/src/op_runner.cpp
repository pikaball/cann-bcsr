...
OpRunner::~OpRunner()
{
    ...
    for (size_t i = 0; i < numInputsArray_; ++i) {
        (void)aclDestroyIntArray(inputArray_[i]);
    }
    for (size_t i = 0; i < numInputs_ - numInputsArray_; ++i) {
...
bool OpRunner::Init()
{
    for (size_t i = 0; i < numInputs_; ++i) {
        ...
        if (i < numInputsArray_) {
            aclIntArray *inputArray =
                aclCreateIntArray(GetInputBuffer<int64_t>(i), GetInputSize(i));
            if (inputArray == nullptr) {
                ERROR_LOG("Create IntArray for input[%zu] failed", i);
                return false;
            }
            inputArray_.emplace_back(inputArray);
        } else {
    ...
    for (size_t i = 0; i < numOutputs_; ++i) {
        ...
        if (aclrtMemset(devMem, size, 0, size) != ACL_SUCCESS) {
            ERROR_LOG("Memset device memory for output[%zu] failed", i);
            return false;
        }
        ...
        // host memory no need to init to zero, as memcpy will overwrite it
